using System;
using System.Collections.Generic;
using System.Diagnostics;

namespace _BFS__DFS_Tree
{
    public class TreeNode<T>
    {
        private T value;
        private bool hasParent;
        private List<TreeNode<T>> children;

        public TreeNode(T value)
        {
            this.Value = value;
            this.children = new List<TreeNode<T>>();
        }

        public T Value
        {
            get { return this.value; }
            set { this.value = value; }
        }

        public int ChildrenCount
        {
            get { return this.children.Count; }
        }

        public void AddChild(TreeNode<T> child)
        {
            if (child.hasParent)
            {
                throw new ArgumentException("node has parent already");
            }
            child.hasParent = true;
            this.children.Add(child);
        }

        public TreeNode<T> GetChild(int index)
        {
            return this.children[index];
        }

    }
    public class Tree<T>
    {
        private TreeNode<T> root;

        public Tree(T value)
        {
            this.Root = new TreeNode<T>(value);
        }

        public Tree(T value, params Tree<T>[] Children)
            : this(value)
        {
            foreach (Tree<T> child in Children)
            {
                this.root.AddChild(child.root);
            }
        }

        public TreeNode<T> Root
        {
            get { return this.root; }
            private set { this.root = value; }
        }

        public void BFS()
        {
            Queue<TreeNode<T>> q = new Queue<TreeNode<T>>();
            q.Enqueue(this.root);
            while (q.Count > 0)
            {
                TreeNode<T> currentNode = q.Dequeue();
                Console.Write($"{currentNode.Value}"+ " ");
                for (int i = 0; i < currentNode.ChildrenCount; i++)
                {
                    TreeNode<T> childNode = currentNode.GetChild(i);
                    q.Enqueue(childNode);
                }
            }
        }
        public void TraverseDFS()
        {
            this.TraverseDFS(this.root, string.Empty);
        }

        private void TraverseDFS(TreeNode<T> root, string empty)
        {
            if(this.root == null)
            {
                return;
            }
            Console.WriteLine(empty + root.Value);

            TreeNode<T> child = null;
            for (int i = 0; i < root.ChildrenCount; i++)
            {
                child = root.GetChild(i);
                this.TraverseDFS(child, empty + "  ");
            }
        }

        public void DFSWithStack()
        {
            Stack<TreeNode<T>> stack = new Stack<TreeNode<T>>();
            stack.Push(this.root);

            while (stack.Count > 0)
            {
                TreeNode<T> currentNode = stack.Pop();
                Console.Write($"{currentNode.Value}"+" ");
                for (int i = 0; i < currentNode.ChildrenCount; i++)
                {
                    TreeNode<T> childNode = currentNode.GetChild(i);
                    stack.Push(childNode);
                }
            }
        }
    }
    class Program
    {
        static void Main(string[] args)
        {
            Tree<int> tree =
                                new Tree<int>(7,
                                              new Tree<int>(19,
                                                            new Tree<int>(1),
                                                            new Tree<int>(12),
                                                            new Tree<int>(31)),
                                              new Tree<int>(21),
                                              new Tree<int>(14,
                                                            new Tree<int>(23),
                                                            new Tree<int>(6)));
            Console.WriteLine("Breadth-First Search (BFS) traversal (with queue):");
            var timeBFS = Time(tree.BFS);
            Console.WriteLine($"Time Taken {timeBFS}");

            Console.WriteLine("Depth-First Search (DFS) traversal (recursive):");
            var timeDFS = Time(tree.TraverseDFS);
            Console.WriteLine($"Time Taken {timeDFS}");

            Console.WriteLine();
            Console.WriteLine("Depth-First Search (DFS) traversal (with stack):");
            var timeDFSStack = Time(tree.DFSWithStack);
            Console.WriteLine($"Time taken {timeDFSStack}");
        }
        public static TimeSpan Time(Action action)
        {
            Stopwatch stopwatch = Stopwatch.StartNew();
            action();
            stopwatch.Stop();
            return stopwatch.Elapsed;
        }
    }
   
}

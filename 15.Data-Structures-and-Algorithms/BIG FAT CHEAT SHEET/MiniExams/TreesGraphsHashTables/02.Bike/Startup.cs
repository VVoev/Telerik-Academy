...
namespace Bike
{
    public class Startup
    {
        ...
        public static void Main()
        {
            ...
            Console.WriteLine("{0:f2}", minDamage);
        }
        ...
        public static double GetSmallestDamage(int rows, int cols)
        {
            ...
            while (!queue.IsEmpty)
            {
                ...
                {
                    // up left
                    if (IsInRange(currentRow - 1, currentCol - 1))
                    {
                        var nextNode = GetNodeNumber(currentRow - 1, currentCol - 1);
                        var weight = CalculateWeight(matrix[currentRow, currentCol], matrix[currentRow - 1, currentCol - 1]);
                        var newDistance = distances[currentNode.Target] + weight;
                        var currentDistance = distances[nextNode];
            ...
            return distances[distances.Length - 1] + Math.Abs(matrix[0,0]) + Math.Abs(matrix[rows - 1, cols - 1]);
        }
